document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateRaceBonuses = (race) => {
        const bonuses = raceBonuses[race] || {strB: 0, dexB: 0, conB: 0, intB: 0, wisB: 0, chaB: 0};
        statBonuses = bonuses;
        Object.keys(bonuses).forEach(key => {
            console.log(`Updating ${key.split('B')[0]}-racial with value ${bonuses[key]}`); // Hangi bonusun güncellendiğini kontrol edin
